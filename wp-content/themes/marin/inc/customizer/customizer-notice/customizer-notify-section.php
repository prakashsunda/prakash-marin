...
class marin_Customizer_Notify_Section extends WP_Customize_Section {

	public $type = 'marin-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'marin_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'marin_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $marin_customizer_notify_recommended_actions as $key => $marin_lite_customizer_notify_recommended_action ) {
			if ( $marin_customizer_notify_show_recommended_actions[ $marin_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $marin_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$marin_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $marin_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $marin_customizer_notify_recommended_action['plugin_slug'] );
				$marin_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $marin_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$marin_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$marin_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$marin_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$marin_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$marin_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$marin_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$marin_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$marin_lite_customizer_notify_recommended_action['button_label'] = $marin_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $marin_lite_customizer_notify_recommended_action;
		}// End foreach().
		...
		foreach ( $marin_customizer_notify_recommended_plugins as $slug => $marin_plugin_opt ) {

			if ( ! $marin_plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $marin_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $marin_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $marin_deactivate_button_label;
					break;
			}
			$marin_info                                    = $this->call_plugin_api( $slug );
			$ti_customizer_notify_recommended_plugin['id'] = $slug;
			$ti_customizer_notify_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $marin_plugin_opt['description'] ) ) {
				$ti_customizer_notify_recommended_plugin['description'] = $marin_plugin_opt['description'];
			} else {
				$ti_customizer_notify_recommended_plugin['description'] = $marin_info->short_description;
			}

			$ti_customizer_notify_recommended_plugin['title'] = $marin_info->name;
		...
		$json['total_actions']       = count( $marin_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="marin-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="marin-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="marin-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="marin-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no marin-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="marin-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="marin-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no marin-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>

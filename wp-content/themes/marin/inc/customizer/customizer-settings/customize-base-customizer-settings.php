...
/**
 * Class marin_Customize_Base_Option
 */
class marin_Customize_Base_Option {
	...
	/**
	 * marin_Customize_Base_Option constructor.
	 */
	public function __construct() {

		// Register customizer options.
		add_action( 'customize_register', array( $this, 'marin_customizer_options' ) );
	...
	protected function get_menu_options() {
		...
		$menu_options         = array();
		$menu_options['none'] = esc_html__( 'None', 'marin' );
	 ...
	 */
	public function marin_customizer_options( $wp_customize ) {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( isset( $control_args['active_callback'] ) && is_array( $control_args['active_callback'] ) ) {
				...
				if ( 'marin_pro' === substr( $cb_setting_id, 0, 9 ) && function_exists( 'marin_pro_options' ) ) {
					$this->ac_default[] = $wp_customize->get_setting( 'marin_pro[' . $cb_setting_id . ']' )->default;
			...
			if ( ! $is_custom_control ) {
				...
				$control_type    = 'marin_Customize_' . $control_type_uc . '_Control';
	...
	public function evaluate() {
		foreach ( $this->active_callback_old[ ( $this->count_evaluate ) ] as $count => $ruleset ) :
			...
			if ( 'marin_pro' === substr( $ac_setting_id, 0, 9 ) && function_exists( 'marin_pro_options' ) ) {
				$option_val = marin_pro_options( $ac_setting_id, 'text_html' );
			} else {

	...
	control: {
		...
		'marin-color': {
			...
			init: function ( control ) {
				...
				// Init the control.
				marin.input.color.init( control );
			},
		...
		color: {
			...
			getTemplate: function ( data ) {
				...
				html +=
					...
					'" value="' +
					data.value +
					'" class="marin-color-control" data-id="' +
					data['data-id'] +
					'"/>';

				return (
					'<div class="marin-input-container" data-id="' +
					data.id +
					'">' +
					html +
					'</div>'
			...
			init: function ( control ) {
				var picker = jQuery(
					'.marin-color-control[data-id="' + control.id + '"]'
				    ),
				    clear;
...
(
	function () {
		 ...
		 */
		wp.customize.marinDynamicControl = wp.customize.Control.extend( {
			initialize: function ( id, options ) {
				...
				if ( !args.params.type ) {
					args.params.type = 'marin-generic';
				}
			...
			ready: function () {
				...
				control.deferred.embedded.done( function () {
					control.initmarinControl( control );
				} );
			},
			...
			embed: function () {
				...
				wp.customize.section( sectionId, function ( section ) {
					if (
						'marin-expanded' === section.params.type ||
						section.expanded() ||
			 ...
			 * @returns {void}
			 */
			initmarinControl: function ( control ) {
				if ( 'undefined' !== typeof marin.control[control.params.type] ) {
					marin.control[control.params.type].init( control );
					return;
				}
			...
			marinValidateCSSValue: function ( value ) {
				var validUnits = [
					    'rem',
					    'em',
					    'ex',
...
_.each( marin.control, function ( obj, type ) {
	wp.customize.controlConstructor[
		type
		] = wp.customize.marinDynamicControl.extend( {} );
} );
 ...
 * Control: Slider.
 */
wp.customize.controlConstructor['marin-slider'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		var control      = this,
		    ...
		    slider       = control.container.find( '.marin-slider' ),
		...
		// Init the control.
		if ( !_.isUndefined( window.marinControlLoader ) && _.isFunction( marinControlLoader ) ) {
			marinControlLoader( control );
		} else {
			control.initmarinControl();
		}
	},

	initmarinControl: function () {
 ...
 * Control: Text.
 */
wp.customize.controlConstructor['marin-text'] = wp.customize.Control.extend( {
 ...
 * Control: Sortable.
 */
/* global marinControlLoader */
wp.customize.controlConstructor['marin-sortable'] = wp.customize.Control.extend( {
	...
	ready: function () {
		...
		// Init the control.
		if (
			!_.isUndefined( window.marinControlLoader ) &&
			_.isFunction( marinControlLoader )
		) {
			marinControlLoader( control );
		} else {
			control.initmarinControl();
	...
	initmarinControl: function () {
		...
		var control = this;
		control.container.find( '.marin-controls-loading-spinner' ).hide();
 ...
 * Control: Radio image.
 */
wp.customize.controlConstructor['marin-radio-image'] = wp.customize.marinDynamicControl.extend( {} );

/**
 * Control: Toggle.
 */
wp.customize.controlConstructor['marin-toggle'] = wp.customize.marinDynamicControl.extend( {
 ...
 */
wp.customize.controlConstructor['marin-radio-buttonset'] = wp.customize.Control.extend( {} );

/**
 * Control: Dimensions.
 */
;
/* global dimensionsmarinL10n */
/* global marinControlLoader */
wp.customize.controlConstructor['marin-dimensions'] = wp.customize.marinDynamicControl.extend( {
			...
			setting.bind( function ( value ) {
				...
				_.each( ['top', 'right', 'bottom', 'left'], function ( direction ) {
					if ( !_.isUndefined( value[direction] ) ) {
						if ( false === control.marinValidateCSSValue( value[direction] ) ) {
							subs[direction] = dimensionsmarinL10n[direction];
						} else {
				...
				if ( !_.isEmpty( subs ) ) {
					message = dimensionsmarinL10n['invalid-value'] + ' (' + _.values( subs ).toString() + ') ';
...
;
/* global marinControlLoader */
wp.customize.controlConstructor['marin-background'] = wp.customize.Control.extend( {
	...
	initmarinControl: function () {
		...
		var control = this,
		    value   = control.setting._value,
		    picker  = control.container.find( '.marin-color-control' );
 ...
 * Control: Editor.
 */
wp.customize.controlConstructor['marin-editor'] = wp.customize.marinDynamicControl.extend( {
	...
	initmarinControl: function () {
		...
		var id = 'marin-editor-' + control_id;
